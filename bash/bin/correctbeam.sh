...
then
    ...
    then
        cat correctbeam_body.template | sed "s;OBSNUM;${obsnum};g" | sed "s;PROJ;${proj};g" | sed "s;DATADIR;${datadir};g" | sed "s;NCPUS;${ncpus};" > correctbeam_${obsnum}.sh
    else
        echo "$datadir/${proj}/${obsnum}/${obsnum}.ms doesn't exist!"
    fi
    bash correctbeam_${obsnum}.sh
else
