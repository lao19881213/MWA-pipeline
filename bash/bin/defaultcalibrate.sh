...
then
    ...
    then
        cat defaultcalibrate_body.template | sed "s;OBSNUM;${obsnum};g" | sed "s;PROJ;${proj};g" | sed "s;DATADIR;${datadir};g" | sed "s;NCPUS;${ncpus};" > defaultcalibrate_${obsnum}.sh
    else
        echo "$datadir/${proj}/${obsnum}/${obsnum}.ms doesn't exist!"
    fi
    bash defaultcalibrate_${obsnum}.sh
else
