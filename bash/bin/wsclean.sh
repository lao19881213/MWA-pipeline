...
then
    ...
    then
        cat wsclean_body.template | sed "s;OBSNUM;${obsnum};g" | sed "s;PROJ;${proj};g" | sed "s;DATADIR;${datadir};g" | sed "s;MODELDIR;${modeldir};g" | sed "s;CALMODEL;${calmodel};g" | sed "s;NCPUS;${ncpus};g" | sed "s;TGTNUM;${tgtobsnum};"  > wscelan_${obsnum}.sh
    else
        echo "$datadir/${proj}/${obsnum}/${obsnum}.ms doesn't exist!"
    fi
    bash wsclean_${obsnum}.sh
else
